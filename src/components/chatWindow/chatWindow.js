...
export default class ChatWindow extends Component {
  ...
  async componentDidMount() {
    var username = this.props.loggedInUserObj.username;
    ...
    try {
     ...
     stompClient.connect({ headers }, frame => {
       ...
       stompClient.subscribe("/topic/" + username, e => {
         ...
         //this.setState({chats : {...this.state.chats, newMessage}})
      ...
      <div className="container flex mx-auto m-2 rounded h-screen bg-white border border-blue-800 bg-gray-100">
        {this.state.channels.length > 0 &&
          <ContactList
            ...
            channelAvatar={this.props.loggedInUserObj.avatar}
          />}
        {this.state.selectedChannel &&
          <MessageBox
            selectedChannel={this.state.selectedChannel}
            loggedInUserAvatar={this.props.loggedInUserObj.avatar}
            loggedInUsername={this.props.loggedInUserObj.username}
