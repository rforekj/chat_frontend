        ...
        <div className=" w-full relative h-full grid grid-flow-rows">
          ...
          <div className="user-bar flex w-full py-4 absolute inset-x-0 top-0 shadow-lg">
            <div className="w-12 rounded-full relative h-12 text-center mx-2">
              <img className="profile-picture absolute h-full object-cover self-center p-2" src={this.props.loggedInUserAvatar} alt="dp" />
            </div>
          ...
          <div className="message-area clearfix overflow-auto my-20 p-2">
            ...
            <div className="outgoing w-3/4 justify-end float-right flex my-2" ref={el => {
                this.messagesEnd = el;
              }} />
          </div>
          ...
          <div className="input-box flex p-4 bottom-0 absolute inset-x-0 bg-white shadow-inner">
            <input className="msg-input p-2 w-4/5 float-left text-sm focus:outline-none focus:ring" placeholder="Write Message.." value={this.state.msgText} onChange={e => this.handleMessageText(e)} onKeyDown={e => this.handleEnterSend(e)} />
