...
export default class MessageBox extends Component {
    ...
    addMessagesToChat() {
        if (this.props.messages) {
            console.log("me " + this.props.messages);
            const msgContent = this.props.messages.map(function (message) {
                if (message.channelId === this.props.selectedChannel.id)
                    return (<div key={message.id} className="outgoing w-3/4 justify-end float-right flex my-2">
                    ...
                    return <div key={message.id} className="incoming w-3/4 flex my-2">
                        <div className="w-16 rounded-full relative h-16 mx-2 px-2">
                          <img className="profile-picture absolute h-full object-cover self-center p-2" src={"/images/" + this.props.selectedChannel.img} alt="dp" />
                        </div>
    ...
    render() {
        return <div className="message-box w-3/5">
            <div className=" w-full relative h-full grid grid-flow-rows">
              ...
              <div className="input-box flex p-4 bottom-0 absolute inset-x-0 bg-white shadow-inner">
                <input className="msg-input p-2 w-4/5 float-left text-sm focus:outline-none focus:ring" placeholder="Write Message.." value={this.state.msgText} onChange={e => this.handleMessageText(e)} />
